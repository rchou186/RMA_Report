# This program collects Mxx.CSV from TEST number and draw DCD and Wh charts
...
def test_build(TEST):
    ...
    sql = f"SELECT * FROM Total WHERE Battery = '{TEST}' ORDER BY Org_Module ASC"
    mycursor.execute(sql)
    myresult = mycursor.fetchall()
    TEST_df = pd.DataFrame(myresult, columns=mycursor.column_names)
    TEST_df.drop('SEQ', axis=1, inplace=True)
    mydb.close()
    ...
    for i in range(0, len(TEST_df)):
        Tnumber = TEST_df.loc[i, 'Battery']
        Org_Module = TEST_df.loc[i, 'Org_Module']
        ...
        destination_file = f"{current_path}/{TEST}/{destination_csv}"
        #print("Copy {0:s} {1:s}.csv to {2:s}.csv".format(Tnumber, Org_Module, destination))
        shutil.copy(source_file, destination_file)

    destination_path = f"{current_path}/{TEST}"
    ...
    fig = px.line(df_all, x='Time', y='Voltage', color='Module', facet_col='Phase', title=TEST+' DCD Chart', color_discrete_map=color_map)
    ...
    print(f"Create bar graph chart: {TEST}_Wh.html")
    fig_bar = make_subplots(rows=1, cols=1, specs=[[{"secondary_y": True}]])
    trace1 = go.Bar(x=TEST_df['Org_Module'], y=TEST_df['CHR_Wh'], name='CHR_Wh', marker={'color': 'DodgerBlue'})
    trace2 = go.Bar(x=TEST_df['Org_Module'], y=TEST_df['DIS_Wh'], name='DIS_Wh', marker={'color': 'mediumseagreen'})
    trace3 = go.Scatter(x=TEST_df['Org_Module'], y=TEST_df['Ratio_Wh'],
    ...
    fig_bar.update_layout(title=f"{TEST} Wh")
